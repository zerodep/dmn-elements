...
| `services`    | named host functions, exposed to FEEL under `services` (e.g. `services.creditScore(Applicant)`) and available through `getServiceByName` — must be synchronous when called from FEEL (a returned promise raises a `DecisionError`), while host-side calls through `getServiceByName` are unguarded and may be async; a variable or evaluation input named `services` shadows the overlay — see the [service example](Examples.md#call-a-service) |
